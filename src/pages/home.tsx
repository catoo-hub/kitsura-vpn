...
const MinimalHomePage: React.FC = () => {
  ...
  const expireInfo = parseExpire(currentProfile?.extra?.expire);

  const statusInfo = useMemo(() => {
    if (isToggling) {
      return {
        text: isProxyEnabled ? t("Disconnecting...") : t("Connecting..."),
        color: isProxyEnabled ? "#c1dceb" : "#44b4f4",
        isAnimating: true,
      };
    }
    if (isProxyEnabled) {
  ...
  return (
    <div className="h-full w-full flex flex-col">
      <div className="absolute inset-0 opacity-30 pointer-events-none z-0 [transform:translateZ(0)]">
        <video
          autoPlay
          loop
          muted
      ...
      {isProxyEnabled && (
        <div
          ...
          style={{
            background:
              "radial-gradient(circle, rgba(69,121,204,0.3) 0%, transparent 70%)",
            filter: "blur(100px)",
          }}
        />
      )}
        ...
        <div className="relative flex flex-col items-center gap-8 py-10 w-full max-w-4xl px-4">
          ...
          <div className="relative text-center">
            ...
            {isProxyEnabled && (
              <div
                className={cn(
                  "absolute top-full left-1/2 -translate-x-1/2 mt-52 flex justify-center items-center",
                  "border bg-background shadow-xs hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 rounded-xl p-2 gap-x-4",
                  "backdrop-blur-sm bg-white/80 border-gray-300/60",
                  "transition-all duration-200",
                )}
              >
...
function parseExpire(expire: any) {
