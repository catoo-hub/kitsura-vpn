...
impl IVerge {
    /// 有效的clash核心名称
    pub const VALID_CLASH_CORES: &'static [&'static str] = &["kitsura-core", "kitsura-core-alpha"];
    ...
    pub fn validate_and_fix_config() -> Result<()> {
        ...
        if needs_fix {
            ...
            help::save_yaml(&config_path, &config, Some("# Kitsura VPN Config"))?;
            logging!(
                info,
                Type::Config,
                true,
    ...
    pub fn get_valid_clash_core(&self) -> String {
        self.clash_core
            .clone()
            .unwrap_or_else(|| "kitsura-core".to_string())
    }
    ...
    pub fn template() -> Self {
        Self {
            clash_core: Some("kitsura-core".into()),
    ...
    pub fn save_file(&self) -> Result<()> {
        help::save_yaml(&dirs::verge_path()?, &self, Some("# Kitsura VPN Config"))
    }
