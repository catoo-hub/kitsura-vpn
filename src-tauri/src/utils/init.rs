...
pub fn init_config() -> Result<()> {
    ...
    crate::log_err!(dirs::clash_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IClashTemp::template().0, Some("# Kitsura VPN"))?;
        }
        <Result<()>>::Ok(())
    }));
    ...
    crate::log_err!(dirs::verge_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IVerge::template(), Some("# Kitsura VPN"))?;
        }
        <Result<()>>::Ok(())
    }));
    ...
    crate::log_err!(dirs::profiles_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IProfiles::template(), Some("# Kitsura VPN"))?;
        }
        <Result<()>>::Ok(())
    }));
...
pub fn init_scheme() -> Result<()> {
    ...
    clash.set_value("", &"Kitsura VPN")?;
    clash.set_value("URL Protocol", &"Kitsura VPN URL Scheme Protocol")?;
