...
!macro CheckAllKitsuraProcesses
  ...
  ; Check if kitsura-core-alpha.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "kitsura-core-alpha.exe"
  !else
    nsis_tauri_utils::FindProcess "kitsura-core-alpha.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill kitsura-core-alpha.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "kitsura-core-alpha.exe"
    !else
      nsis_tauri_utils::KillProcess "kitsura-core-alpha.exe"
    !endif
  ${EndIf}

  ; Check if kitsura-core.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "kitsura-core.exe"
  !else
    nsis_tauri_utils::FindProcess "kitsura-core.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill kitsura-core.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "kitsura-core.exe"
    !else
      nsis_tauri_utils::KillProcess "kitsura-core.exe"
    !endif
  ${EndIf}
...
Section Uninstall
  ...
  ; 兼容旧名称快捷方式
  !insertmacro UnpinShortcut "$SMPROGRAMS\$AppStartMenuFolder\kitsura-vpn.lnk"
  !insertmacro UnpinShortcut "$DESKTOP\kitsura-vpn.lnk"

  ; Remove start menu shortcut
  !insertmacro MUI_STARTMENU_GETFOLDER Application $AppStartMenuFolder
  Delete "$SMPROGRAMS\$AppStartMenuFolder\${PRODUCTNAME}.lnk"
  ; 兼容旧名称快捷方式
  Delete "$SMPROGRAMS\$AppStartMenuFolder\kitsura-vpn.lnk"
  RMDir "$SMPROGRAMS\$AppStartMenuFolder"

  ; Remove desktop shortcuts
  Delete "$DESKTOP\${PRODUCTNAME}.lnk"
  ; 兼容旧名称快捷方式
  Delete "$DESKTOP\kitsura-vpn.lnk"
